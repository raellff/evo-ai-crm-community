module LabelConcern
  ...
  # Fallback for callers that pass a single label under a singular key (or a
  # bare scalar `labels`) rather than the canonical `labels` array.
  # `params.permit` here keeps the input filtered (scalar string only).
  ...
  #
  # EVO-1897 (D8): a UUID-shaped token that does NOT resolve to a `Label` row
  # (e.g. a journey/evo-flow caller carrying an id that is absent from the
  # local `labels` table) must NOT be silently dropped. Doing so made
  # `update_labels([])` wipe the list and reply `200` with no tagging
  # persisted — a false-success for add-label/remove-label nodes. Preserve the
  # original token when it cannot be resolved so the caller's intent is always
  # reflected as a real tagging.
