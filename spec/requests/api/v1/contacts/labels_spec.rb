...
# persist a tagging whenever the caller expresses a label, including the shapes
# used by the journeys/evo-flow add-label node — by NAME under a singular
# `labelId` key, a bare scalar, or a UUID (resolved or orphan). It previously
# replied `200` with no tagging persisted (a false-success) for those shapes.
# The node authenticates server-side with the service token, so the request is
# exercised through that path here.
RSpec.describe 'Api::V1::Contacts::Labels', type: :request do
  ...
  describe 'POST /api/v1/contacts/:contact_id/labels' do
    ...
    # Regression (EVO-1897): a UUID-shaped token that does not resolve to a
